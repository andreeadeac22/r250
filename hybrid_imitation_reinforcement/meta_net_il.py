...
class MetaNN:
    def __init__(self):
        ...
        with tf.device('/cpu:'+str(GPU)):
            ...
            self.meta_controller.add(Dense(HIDDEN_NODES, activation = 'relu', kernel_initializer = initializers.random_normal(stddev=0.01, seed = SEED), input_shape = (4,)))
            self.meta_controller.add(Dropout(0.5))
    ...
    def collect(self, processed, expert_a):
        if processed is not None:
            self.replay_hist[self.ind] = (list(map(np.float32, processed)), list(map(np.float32, expert_a)))
    ...
    def train(self):
        ...
        self._train_y = np.reshape([self._selected_replay_data[i][1] for i in range(BATCH_SIZE)],(BATCH_SIZE,nb_Option))
    ...
    def predict(self, x, batch_size=1):
        ...
        print("x in predict ", x)
        return self.meta_controller.predict([np.reshape(x, (1, 4))], batch_size=batch_size, verbose=0)[0]
